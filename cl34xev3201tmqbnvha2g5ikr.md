...
Like most other blockchains, Koinos has a testnet available, so we're going to use it to create a test account, top it up with some `tKoin`, which is the native test token of the blockchain, and deploy our smart contract.
...
- download the `.zip` file that's compatible with your system, in my case, it is the zip file called `koinos-cli.v0.3.1.osx.zip`
...
## Generate a wallet
In order to interact with the Koinos blockchain we need to have a wallet. To create one, simply type the following commands:
...
- `azerty` in this case is the password for my wallet, this password is basically used to encrypt your wallet file on your computer. (The wallet file contains the private key of the wallet)
...
Now that we have generated a wallet, let's top it up with some `tKoin`!
To do that you'll need to have a Discord account. If you don't have one, you can easily create one at https://discord.com/register
...
You might be asked to introduce yourself or go through some anti-bot verification.
Once the verification completed, you should be able to have access to the Koinos Discord channels, and there's one channel in particular that we're going to use, it's called `faucet` and it's located in the `Testnet` category:
...
Now, let's wait a few seconds and let's go back to our terminal where our `koinos-cli` and our wallet is open and type the following command:
...
This command will connect the CLI to a Koinos RPC node, in this case it's the Koinos node.
Then type:
```bash
balance
```
...
100 mana
```
If not, just wait a couple of minutes, transaction don't happen instantaneously so it might take some time before it will show the correct balance.
...
where `0x122096042fb2e2c085eb4e78cb80a4933e6cda21ebf65722e2c0f283f39a9ba40f2a` is the transaction ID that was printed in our terminal.
If you get a `500 Internal Server Error.` this means that either you didn't type the transaction ID correctly, or the transaction hasn't been processed yet. Just reload until the transaction details show up.
This is what shows up for my transaction:
...
```
where:
- `register` is the command used to register the contract with the CLI. Register basically tells the CLI that you want to load the ABI file of a smart contract
...
By default, smart contracts are upgradeable on Koinos! To do so, it's super simple, just follow the same steps we used to upload the contract, which basically means, you just need to use the `upload` command and pass the path to new WASM and ABI file as arguments.
